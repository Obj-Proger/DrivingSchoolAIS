...
internal sealed class CreateTestTemplateCommandValidator
    : AbstractValidator<CreateTestTemplateCommand>
{
    public CreateTestTemplateCommandValidator()
    {
        ...
        When(x => x.IsAutoAssigned, () =>
        {
            RuleFor(x => x.AutoAssignEveryNLessons)
                .NotNull().WithMessage("Auto-assign interval is required when auto-assign is enabled.")
                .GreaterThan(0).WithMessage("Auto-assign interval must be greater than zero.");
        });
    }
}